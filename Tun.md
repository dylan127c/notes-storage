...
判定目标一旦命中规则，则会按照规则指定的模式发起网络请求。常见的请求模式有三种：

- DIRECT：直连模式。将使用 DNS 配置中的解析服务来解析域名，并根据分流规则（fallback-filter）发起最终的网络请求；
...
- 判定目标如果匹配到规则列表 direct，那么所有流量都会使用直连模式（DIRECT）；
- 判定目标如果匹配到规则列表 proxy，那么所有流量都会使用代理模式（PROXY）；
- 判定目标如果匹配到规则列表 reject，那么所有流量都会使用拒绝模式（REJECT）。 
...
换言之，只有判定目标所匹配到的模式为 DIRECT 直连模式时，才需要使用 DNS 解析服务。
...
这种情况下，使用 IP 规则就能直接判断该 IP 应该使用代理模式、直连模式还是拒绝模式。
...
这种糟糕的解析体验，同时也影响着使用 DIRECT 直连模式的域名。

使用 DIRECT 直连模式的域名经解析后，最终还需要匹配 fallback-filter 分流规则：
...
但由于高可用的国外 DNS 服务器几乎不存在，如果目标是国外域名，那最终总是会使用 fallback 中的服务器发起网络请求，其结果也结果几乎必为“连接不可用”。
...
注意： fallback-filter 字段实际是 clash-core 中自带的规则字段，即便配置中省略了，但实际该规则仍然内置、无法被移除。
...
1. 53 端口将持续处于被监听状态，多开 Clash 可能会出现端口监听失败的错误日志；
2. nameserver 和 fallback 中的 DNS 会被用于解析规则模式下其要求解析的域名，同时本地 DNS 会被用于解析一些未经 Clash 接管的网络流量的解析请求。

为了避免未知错误和优化系统性能，推荐默认情况下将 dns.enable 的值设置为 false。

启用 TUN 模式时，dns.enable 的值会被自动设置为 true，无须担心配置可能出错的问题。

手机、平板等移动设备的代理网络如果涉及到 DNS 配置，则推荐将 dns.enable 设置为 true。

因为大多移动设备上的代理 APP 程序，其实现代理上网的模式通常就是 TUN 模式。

关于 CFW 中配置文件内存在的 DNS 配置的说明：
