...
针对于不支持代理服务的软件来说，目前用于实现它们使用代理服务的主流手段是 TUN 模式，具体的说明和实践可以参考 [TUN](./TUN.md)。

简单来说，TUN 模式依赖于支持 TUN 模式的代理软件，启用 TUN 模式能够在本地部署虚拟网卡，以接管本地计算机上产生的几乎所有的网络流量。

这样即便是不支持代理服务的软件，其网络流量也能够被虚拟网卡接管，以强制使用代理服务。

TUN 模式虽然能够让本身不支持代理服务的软件使用代理，但其本身并不完美。因其劫持网络流量的实现特性，不可避免地会让一些本身不需要使用代理的的软件流量被劫持，从而影响请求效率。
