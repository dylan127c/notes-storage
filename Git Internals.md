...
Git 的主要目的是跟踪和管理文件的版本历史，它的实现方式在很大程度上参考了数据库设计的概念。Git 内部使用了一种被称为对象数据库（Object Database）的结构来存储和管理数据。

对象数据库是一种键-值存储系统，其中：

- 键（key）：对象的 SHA-1 校验和（checksum）；
- 值（value）：对象的数据经压缩后得到的数据文件。
...
相较于传统的对象数据库，Git 更专注于版本控制和跟踪文件的变化，而不提供广泛的查询和操作数据的功能。Git 仅在某种程度上与数据库有所关联，因此通常不会直接将 Git 描述为数据库。
...
Git 同样使用 Blob 对象来存储文件内容，该对象的大致结构如下：

<img src="images/Git%20Internals.images/object-blob.png" alt="img" style="zoom:80%;" />

其中，首行被称为 Header 头部信息，其余部分则被称为 Content 文件内容。

Header 头部信息由以下字段组成：

- 类型（Type）：即对象类型。于 Blob 对象来说，该信息为“blob”；
- 大小（Size）：即文件内容的大小。内容大小以字节（byte）为单位。

Header 信息中类型和大小两者之间，需要使用空格或制表符进行分割。以 Blob 对象为例，Header 的书写格式为：

```
blob content.bytesize\u0000
```

信息的前段为 Type，后段为 Size：
...
对象中的 Header 和 Content 之间，通常也需要使用字符进行分割，但并非使用空格或制表符，取而代之使用的是 NULL 空字符。以下是 Blob 对象的基本格式：
