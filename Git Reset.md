...
所有添加到暂存区的文件，其文件内容都会以 Blob 对象的形式**永久存在**。以下命令用于添加文件到暂存区：

```bash
git add <file>…
```

文件添加到暂存区后，Git 会根据该文件的内容生成对应的 Blob 对象，并将这些对象存储在 .git/objects 目录中。

Blob 对象并不是直接存放在 .git/object 目录中，Git 会先创建一个用于存储该对象的目录，之后再将对应的 Blob 对象存储到该新建目录中。新建目录通常以目标文件内容的 SHA-1 校验和的前 2 位字符命名，对应的 Blob 对象则使用 SHA-1 校验和的后 38 位字符命名。

上文提到了**永久存在**，这意味着文件一旦被添加到暂存区，即便使用以下命令取消暂存：
...
目标文件内容所对应的 Blob 对象也不会被删除，它将永久被存储在 .git/objects 目录中。即在项目的后续版本中，如果你添加到暂存区的文件已存在对应的 Blob 对象，那么 Git 将不会再生成一个新的 Blob 对象，取而代之它会使用旧的、已存在的 Blob 对象以记录目标文件的内容。

Blob 对象仅根据**文件内容**来计算对应的 SHA-1 校验和，这种类型的对象通常用于存储内容。这意味着，即便文件拥有不同的命名，只要相同内容，那么它们所得到的 SHA-1 校验和也会相同，因为它们都会指向同一个 Blob 对象。

如果已添加到暂存区的文件后续被修改了内容，那么 Git 也仅是会将该文件指向一个新的 Blob 对象。同时，原本旧的 Blob 对象也会被永久存储在 .git/objects 目录中，不会自动移除，即便没有任何文件指向该 Blob 对象。

所谓版本回溯，不过是将指定版本中的文件恢复，并将其指向原本的、旧的 Blob 对象。多数情况下，版本回溯并不会破坏工作区的文件，因此对已跟踪文件的修改记录也能够得以保留。
