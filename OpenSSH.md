...
3. **文件传输**：除了远程登录，OpenSSH 还支持安全的文件传输协议（SFTP），允许用户在本地计算机和远程服务器之间传输文件；
...
然而实际工作中，一般很少会接触到 SSH 协议，更不用说是使用 OpenSSH 来建立 SSH 连接了。多数情况下，与目标服务器建立连接会更倾向于使用 HTTP(s) 协议，因为使用 HTTP(s) 连接时，完成身份验证一般只需要简单地提供用户名称、密码。
...
Windows 系统自 Windows 10 版本 1709 以及更高版本开始，也支持 OpenSSH 客户端和 OpenSSH 服务端等功能，具体安装步骤可以查看：[使用 Windows 设置来安装 OpenSSH](https://learn.microsoft.com/zh-cn/windows-server/administration/openssh/openssh_install_firstuse?tabs=gui#install-openssh-using-windows-settings)。

如果仅是想连接远程的 SSH 服务，那么只选择安装 OpenSSH 客户端功能足矣：

<img src="images/OpenSSH.images/image-20230902153321103.png" alt="image-20230902153321103" style="zoom: 50%;" />

Windows 系统下的 OpenSSH Client 服务安装完毕后，OpenSSH Client 根目录会自动添加到（系统的）环境变量中：

<img src="images/OpenSSH.images/image-20230902153711122.png" alt="image-20230902153711122" style="zoom:50%;" />

从环境变量中可以得知 OpenSSH Client 的根目录路径：

<img src="images/OpenSSH.images/image-20230902153805088.png" alt="image-20230902153805088" style="zoom:50%;" />

由于 OpenSSH Client 根目录已被添加到环境变量，因此用户可在终端中直接使用相关的 SSH 命令。例如，查看当前的 OpenSSH 版本：
...
`ssh-keygen` 本身没有提供类似于 `--help` 这样的参数，Windows 系统下可选择参阅网络文档 [ssh-keygen(1) - Linux man page](https://linux.die.net/man/1/ssh-keygen)。

或选择错误地使用 `ssh-keygen` 命令，在错误使用命令时终端会输出关于命令的用法文档：

<img src="images/OpenSSH.images/image-20230903025515439.png" alt="image-20230903025515439" style="zoom:50%;" />

以下是一些常用的 `ssh-keygen` 参数及说明：
...
这些参数可以组合使用，以满足您的具体需求。例如，要生成一个密码保护的 2048 位长的 RSA 密钥，并将其保存在指定路径下，可以使用以下命令：
...
`ssh` 用于建立和管理 SSH（Secure Shell）连接，在了解该命令之前，需要知道具体使用 SSH 协议的链接格式是怎样的。
...
大多数情况下能够获取到的 SSH 链接都采用 scp 式的简写形式。假如 GitHub 上有一个 dylan127c/sample 仓库，那么它所对应的 SSH 链接就是：

```
git@github.com:dylan127c/sample.git
```

由于 `ssh` 命令仅用于建立和管理 SSH 连接，因此它仅关心是否能与服务端建立连接，而不关心具体需要连接的是哪个仓库。因此 `ssh` 命令仅需要使用到 SSH 链接中的前半部分，即包含用户名称、服务端地址等信息足矣。
...
以下是一个用于与 GitHub 服务器建立连接的 OpenSSH 配置：
