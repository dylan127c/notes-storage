...
但本篇中，**将固定让 HTTP(S) 协议的代理服务作为白名单模式，同时让 SOCKS5 协议的代理服务作为黑名单模式。**

### HTTP(S) 协议选择

关于软件或工具中配置 HTTP(S) 代理服务的协议选择问题。
...
其中，ProxyCap 仅支持以 HTTPS 协议形式来使用目标本地的 HTTP(S) 协议的代理服务；SwitchyOmega 和 Axios 等则仅支持以 HTTP 协议形式来使用本地的代理服务。

不仅协议方面有所差异，某些软件或工具的入站流量（IN-TYPE）类型也与预期的不一致。例如 SwitchyOmega 明明以 HTTP 协议形式来使用本地的代理服务，但其转发至代理服务中的流量类型却为 HTTPS。

这个现象着实让人费解，遗憾的是关于此问题暂时没有一个较为靠谱的解答。

因此，在配置软件或工具的代理服务时，最好依据实际情况来选取正确的 HTTP(S) 协议；同样，软件或工具流入代理服务中的实际流量类型，需查阅代理服务日志的明细后再进行判定。
...
因此，这里会使用 Clash for Windows 作为前置代理以分流并转发某些特殊软件的流量，权当 SUB-RULE 使用。

```yaml
- PROCESS-NAME,clash-win64.exe,PROXY
```
...
下载工具一般支持配置代理服务，其使用代理服务时仅需要考虑一个关键问题：如何让命中的国外流量使用同一个策略。

**因为下载工具如若需要使用代理服务，则该代理服务必须是能够承担大流量下载任务的。在拥有多个不同的代理服务提供商的前提下，为不同的场景选用不同的服务提供商是非常有必要的。**

实际上，子规则分流能够完美地让下载工具中命中的国外流量使用同一个策略。

假如 SUB-RULE 规则可用，以 PikPak 网盘为例：
...
sub-rules:
  pikpak:
    - RULE-SET,addition-reject,REJECT
    - RULE-SET,addition-direct,DIRECT
    - RULE-SET,special-pikpak,PROXY
    - RULE-SET,addition-proxy,PROXY
    - RULE-SET,original-applications,DIRECT
    - RULE-SET,original-apple,DIRECT
    - RULE-SET,original-icloud,DIRECT
    - RULE-SET,original-private,DIRECT
    - RULE-SET,original-direct,DIRECT
    - RULE-SET,original-greatfire,PROXY
    - RULE-SET,original-gfw,PROXY
    - RULE-SET,original-proxy,PROXY
    - RULE-SET,original-tld-not-cn,PROXY
    - RULE-SET,original-reject,REJECT
    - RULE-SET,original-telegramcidr,PROXY,no-resolve
    - RULE-SET,original-lancidr,DIRECT,no-resolve
    - RULE-SET,original-cncidr,DIRECT,no-resolve
    - GEOIP,LAN,DIRECT,no-resolve
    - GEOIP,CN,DIRECT,no-resolve
...
但由于 Meta 内核存在代码错误，子规则 SUB-RULE 并无法使用，这里采用前置代理来模拟子规则。

本篇的前置代理为 Clash for Windows 工具，将原本的子规则配置为前置代理的规则，并替换其中的 PROXY 规则为包含目标代理服务的策略即可：
...
由于分流已经在前置代理中执行过一次了，真实的代理服务中不必再次执行分流，因此以上规则通常要置于规则顶部。

PikPak 不再使用真实的代理服务作为代理，而需要将前置代理配置为代理，所有来自于 PikPak 的流量都会使用前置代理中的分流规则。如若命中规则，则流量将使用预设的策略进行处理，未命中流量则默认使用 DIRECT 策略。

类似于 IDM 等下载工具中均可以配置这个前置代理，以分流来自国外的下载链接请求，让代理服务来完成这些下载任务。

除开浏览器和下载工具等较为特殊的软件之外，其他软件可直接分类为不支持代理服务的软件和支持代理服务的软件。
...
另外类似于 Node.js 这种网络框架，同样推荐使用 ProxyCap 来转发流量至代理服务。Node.js 中需要使用代理服务的无非就是 npm 包管理工具，因为多数的 npm 仓库位于国外，拉取仓库超时是常有的事。
